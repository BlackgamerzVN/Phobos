...
- When it exists, it can increase the power amount generated by the power plants.
  ...
  - `PowerPlantEnhancer.Amount.<PowerPlantID>` and `PowerPlantEnhancer.Factor.<PowerPlantID>` can be used to override `PowerPlantEnhancer.Amount` / `PowerPlantEnhancer.Factor` per specific power plant listed in `PowerPlantEnhancer.PowerPlants`. Any plant without a per-plant override falls back to the scalar value. Per-plant keys for plants not listed in `PowerPlantEnhancer.PowerPlants` are ignored.
...
PowerPlantEnhancer.Amount.<PowerPlantID>=            ; integer, per-plant override for .Amount
PowerPlantEnhancer.Factor.<PowerPlantID>=            ; floating point value, per-plant override for .Factor
```
